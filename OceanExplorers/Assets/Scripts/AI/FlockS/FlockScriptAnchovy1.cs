...
using UnityEngine;
//script controls the flocks movements and holds the boids algorithm
public class FlockScriptAnchovy1 : MonoBehaviour
{
    [HideInInspector]
    public FlockManScriptAnchovy1 fishManager; //connection to flock manager
        ...
        foreach (GameObject fish in fishArray) //loop through all the fish
        {
            if (fish != this.gameObject) //if the current fish isnt the same as the fish connected to  this script right now
            {
                ...
                if (distance <= fishManager.data.fishDistance) //if under the set distance for neighbour fish
                {
                    ...
                    FlockScriptAnchovy1 anotherFlock = fish.GetComponent<FlockScriptAnchovy1>(); //get the script on that fish
