...
//anchovy1
public class FlockManScriptAnchovy1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //foreach (GameObject fish in data.fishprefab) { //for every prefab in the array
                ...
                allFish = new GameObject[data.fishAmount[0]]; //set array size to the amount of fish of that prefab
                for (int i = 0; i < data.fishAmount[0]; i++) { //loop through all those fish
                   ...
                   // if (data.fishprefab[i] != null) { // overiding to stop the headset lagging out
                            ...
                            allFish[i] = (GameObject)Instantiate(data.fishprefab[0], position, Quaternion.identity); //create an instance of that prefab
                            allFish[i].GetComponent<FlockScriptAnchovy1>().fishManager = this; //link the flock script to this script
                       // }
                    }
                }
