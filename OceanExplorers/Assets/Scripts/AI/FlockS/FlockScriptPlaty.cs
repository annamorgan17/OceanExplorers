...
public class FlockScriptPlaty : MonoBehaviour
{
    [HideInInspector]
    public FlockManScriptPlaty fishManager; //connection to flock manager
    ...
    void Update()
    {
        ...
        Bounds b = new Bounds(fishManager.setPoint, fishManager.data.swimLimits * 2); //creates a new bound the size of the swim limit
        ...
        if (Random.Range(0, fishManager.data.randomAmount) < 100) // randomly create bubble and play sound effect
        {
            Bubbles(fishManager.data.bubblePrefab);
            // bubble.transform.parent = this.transform; //creates the fish as the bubble parent
    ...
    private void ApplyRules()
    {
        ...
        Vector3 center = fishManager.setPoint; //set point acting as 0
        ...
        foreach (GameObject fish in fishArray) //loop through all the fish
        {
            if (fish != this.gameObject) //if the current fish isnt the same as the fish connected to  this script right now
            {
                ...
                if (distance <= fishManager.data.fishDistance) //if under the set distance for neighbour fish
                {
                    ...
                    FlockScriptPlaty anotherFlock = fish.GetComponent<FlockScriptPlaty>(); //get the script on that fish
        ...
        if (groupSize > 0) //if there is a group
        {
            ...
            if (direction != fishManager.setPoint) //if the direction isnt the set point
