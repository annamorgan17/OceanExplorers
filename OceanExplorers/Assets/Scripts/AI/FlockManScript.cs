...
public class FlockManScript : MonoBehaviour
{
    ...
    { 
            foreach (GameObject fish in data.fishprefab) { //for every prefab in the array
                ...
                allFish = new GameObject[data.fishAmount[counter]]; //set array size to the amount of fish of that prefab
                for (int i = 0; i < data.fishAmount[counter]; i++) { //loop through all those fish
                    ...
                    if (allFish[i] != null) { // overiding to stop the headset lagging out
                            ...
                            allFish[i] = (GameObject)Instantiate(fish, position, Quaternion.identity); //create an instance of that prefab
                            allFish[i].GetComponent<FlockScript>().fishManager = this; //link the flock script to this script
                        }
                    }
                }
