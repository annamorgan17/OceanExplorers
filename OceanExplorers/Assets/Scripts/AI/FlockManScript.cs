...
public class FlockManScript : MonoBehaviour
{
    ...
    void Start()
    {
        data.setPoint = this.transform.position;
            ...
            { //loop through all those fish
                ...
                { // overiding to stop the headset lagging out
                  // if (data.fishprefab[i] != null) { // overiding to stop the headset lagging out
                    ...
                    allFish[i] = (GameObject)Instantiate(data.fishprefab[counter], position, Quaternion.identity); //create an instance of that prefab
