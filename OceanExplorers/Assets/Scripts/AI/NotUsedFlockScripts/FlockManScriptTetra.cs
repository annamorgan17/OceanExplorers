...
//tetra
public class FlockManScriptTetra : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        allFish = new GameObject[data.fishAmount[4]]; //set array size to the amount of fish of that prefab
        for (int i = 0; i < data.fishAmount[4]; i++)
        { //loop through all those fish
            ...
            { // overiding to stop the headset lagging out
              // if (data.fishprefab[i] != null) { // overiding to stop the headset lagging out
                ...
                allFish[i] = (GameObject)Instantiate(data.fishprefab[4], position, Quaternion.identity); //create an instance of that prefab
                allFish[i].GetComponent<FlockScriptTetra>().fishManager = this; //link the flock script to this script
                                                                           // }
            }
        }
