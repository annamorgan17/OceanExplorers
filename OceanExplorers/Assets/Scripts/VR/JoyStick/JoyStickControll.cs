...
public class JoyStickControll : MonoBehaviour {
    ...
    void Update() {
        //calculating the forwards and backwards for earlier debugging
        ...
        //invoking the lever up when it reaches the position
        if (transform.eulerAngles.x >= 275) { 
            OnLeverUp?.Invoke();
        }

        //debug side to side mode when lever mode is not active
    ...
    //when the hand collides
    private void OnTriggerStay(Collider other) {
        //is the hand
        if (other.CompareTag("PlayerHand")) {
            //find new position and store old and new
            ...
            if (leverMode) { // limit right and left direction if it is acting like a lever rather than a joystick 
                if (newEuler.x < 30 & newEuler.x > 0) { 
                    //stopping it going out of bounds in this use case
                    newEuler.x = 0;
                } else {
                    //Debug.LogError("ss");
                } 
