...
public class OBJLoader
{
    ...
    public static Material[] LoadMTLFile(string fn)
    {
        ...
        foreach (string ln in File.ReadAllLines(fn))
        {
            ...
            else if (cmps[0] == "map_Kd")
            {
                //TEXTURE
                string tpth = cmps[1];
                string fpth = OBJGetFilePath(tpth, mtlFileDirectory);
                if (fpth != null)
    ...
    public static GameObject LoadOBJFile(string fn)
    {
        ...
        //populate object names with a default
        ...
        foreach (string ln in File.ReadAllLines(fn))
        {
            if (ln.Length > 0)
            {
                ...
                if (cmps[0] == "mtllib")
                {
                    ...
                    if (cmps[1].Contains("\\") || cmps[1].Contains("/"))
                    {
                        absolutePath = true;
                    }
                    string pth = cmps[1];
